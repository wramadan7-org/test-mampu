...
import { Post } from "@/app/users/_type/post";
...
export default async function Page({ params }: Props) {
  ...
  const filteredPosts = posts?.[0]?.id
    ? posts
  ...
  return (
    <div className="flex flex-col flex-1 items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex flex-1 w-full max-w-7xl flex-col items-center justify-start py-6 sm:py-12 px-3 sm:px-16 bg-white dark:bg-black sm:items-start gap-4">
        ...
        <h1 className="text-3xl font-bold mr-auto">User Posts</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredPosts?.map((post) => (
            <div
              key={post.id}
              className="border border-neutral-800 rounded-xl p-4 shadow-sm hover:shadow-md transition"
            >
              <h3 className="font-semibold line-clamp-2">{post.title}</h3>
              <p className="text-sm text-neutral-500 mt-2 line-clamp-5">
                {post.body}
              </p>
            </div>
          ))}
